...
metadata <- read_tsv("C:/Users/DrPanks/Google Drive/Metaanalysis/data/trim_GSE20271.tsv")

# Read in data TSV file
df<- read_tsv("C:/Users/DrPanks/Google Drive/Metaanalysis/data/GSE20271/GSE20271.tsv") %>%
...
metadata_GSE20271 <- metadata %>% 
  filter(Race %in% c("Black", "White")) %>%
  filter(!is.na(Race))

#get sample IDs
common_SampleID <- pull(metadata_GSE20271, SampleID)

#make new dataframe with commom sampleIDs
df_GSE20271 <- df[, common_SampleID] %>%
