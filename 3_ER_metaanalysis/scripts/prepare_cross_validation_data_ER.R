...
cross_validation_data <- "/Data/cross_validation_data/ER_status"

if (!dir.exists(cross_validation_data)) {
  dir.create(cross_validation_data, recursive = TRUE)
}

metadata_dir <- "/Data/metadata/ER_status/"
...
# results_dir <- "/Data/results/ER_status/"
...
for (i in 1:length(file_paths_meta)) {
    ...
    identifiers <- meta_data |>
        dplyr::select(Sample_ID, ER_status) |> 
        as_tibble()
    ...
    ER_status_tally[[i]] <- num_par
    names(ER_status_tally)[i] <- filename
}

ER_summary <- bind_rows(ER_status_tally, .id = "Dataset") # convert list to tibble
write_tsv(ER_summary, file.path("/Data/results", "ER_summary.tsv"))
