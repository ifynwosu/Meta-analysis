...
cross_validation_full_results_dir <- "/inwosu/Meta_Analysis/Data/cross_validation_full"

if (!dir.exists(cross_validation_full_results_dir)) {
  dir.create(cross_validation_full_results_dir)
}
...
for (i in 1:length(file_paths_meta)) {
  ...
  write_tsv(joined_data, file.path(cross_validation_full_results_dir, paste0(filename, ".tsv")))
}
