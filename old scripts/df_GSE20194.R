...
metadata <- read_tsv("C:/Users/DrPanks/Google Drive/Metaanalysis/data/trim_GSE20194.tsv")

# Read in data TSV file
df<- read_tsv("C:/Users/DrPanks/Google Drive/Metaanalysis/data/GSE20194/GSE20194.tsv") %>%
...
metadata_GSE20194 <- metadata %>% 
  filter(Race %in% c("Black", "White")) %>%
  filter(!is.na(Race))

#get sample IDs
common_SampleID <- pull(metadata_GSE20194, SampleID)

#make new dataframe with commom sampleIDs
df_GSE20194 = df[, common_SampleID] %>%
