...
metadata <- read_tsv("C:/Users/DrPanks/Google Drive/Metaanalysis/data/trim_GSE24185.tsv")

# Read in data TSV file
df<- read_tsv("C:/Users/DrPanks/Google Drive/Metaanalysis/data/GSE24185/GSE24185.tsv") %>%
...
metadata_GSE24185 <- metadata %>% 
  filter(Race %in% c("Black", "White")) %>%
  filter(!is.na(Race))

#get sample IDs
common_SampleID <- pull(metadata_GSE24185, SampleID)

#make new dataframe with commom sampleIDs
df_GSE24185 = df[, common_SampleID] %>%
