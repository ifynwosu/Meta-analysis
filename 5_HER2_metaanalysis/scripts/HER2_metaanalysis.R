...
HER2_meta_results_dir <- "/inwosu/Meta_Analysis/Data/HER2_meta_results"

if (!dir.exists(HER2_meta_results_dir)) {
  dir.create(HER2_meta_results_dir, recursive = TRUE)
}

# directory where metadata is stored
meta_dir <- "/inwosu/Meta_Analysis/Data/HER2_metadata"  
meta_dir_paths <- list.files(meta_dir, full.names = T)
meta_list <- list()

# directory where expression data is stored
expr_dir  <- "/inwosu/Meta_Analysis/Data/HER2_expression_data"
...
phenoGroups = rep("HER2_Status", length(combinations[,1]))
...
dataset_vector = str_replace_all(expr_dir_paths, c("/inwosu/Meta_Analysis/Data/HER2_expression_data/" = "", ".tsv.gz" = ""))
