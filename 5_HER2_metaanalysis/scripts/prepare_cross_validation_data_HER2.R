...
cross_validation_data <- "/Data/cross_validation_data/HER2_status"

if (!dir.exists(cross_validation_data)) {
  dir.create(cross_validation_data, recursive = TRUE)
}

metadata_dir <- "/Data/metadata/HER2_status/"
file_paths_meta <- list.files(metadata_dir, full.names = T)

expr_dir <- "/Data/expression_data/HER2_status/"
...
for (i in 1:length(file_paths_meta)) {
    ...
    HER2_status_tally[[i]] <- num_par
    names(HER2_status_tally)[i] <- filename
}

HER2_summary <- bind_rows(HER2_status_tally, .id = "Dataset") # convert list to tibble
write_tsv(HER2_summary, file.path("/Data/results", "HER2_summary.tsv"))
