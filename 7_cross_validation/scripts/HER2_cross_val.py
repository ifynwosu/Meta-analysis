...
accuracy_file = os.path.join(result_dir, "accuracy_file_HER2_status.txt")
accuracy_file_results = os.path.join(result_dir, "accuracy_file_result_HER2_status.txt")
...
for num in num_genes:
    ...
    for filename in os.listdir(meta_results_dir):    
        ...
        X = cross_val_df[gene]
        y = cross_val_df['HER2_status'].map({'negative':0, 'positive':1})
        ...
        for i in range(num_cv_runs):             
            genes_only = cross_val_df.drop('HER2_status', axis = 1)
            ...
            y_random = cross_val_df['HER2_status']      
            ...
            # report performance  
            print(dataset_id, "number of genes:" , num, "iteration:", i, ' Random genes accuracy: %.3f (%.3f)' % (random_mean_score, random_std_dev))        
    ...
    final_results = os.path.join(result_dir, "HER2_results_df.tsv")
