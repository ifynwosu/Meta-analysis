...
trip_neg_results_dir <- "/Data/results/trip_neg"

if (!dir.exists(trip_neg_results_dir)) {
  dir.create(trip_neg_results_dir, recursive = TRUE)
}

# directory where metadata is stored
meta_dir <- "/Data/metadata/triple_negative" 
meta_dir_paths <- list.files(meta_dir, full.names = T)
meta_list <- list()

# directory where expression data is stored
expr_dir  <- "/Data/expression_data/triple_negative"
...
dataset_vector = str_replace_all(expr_dir_paths, c("/Data/expression_data/triple_negative/" = "", ".tsv.gz" = ""))

# create vectors for phenotype data
phenoGroups = rep("result", length(combinations[,1]))
phenoCases = rep(list("tri_neg"), length(combinations[,1]))
phenoControls = rep(list("non_tri_neg"), length(combinations[,1]))
...
for (i in seq_along(list_all_exprdata)) {
  ...
  write_tsv(new_results, file.path(trip_neg_results_dir, paste0("meta_results_without_", missing_dataset, ".tsv")))
}

common_meta_genes <- tibble(Reduce(intersect, meta_analysis_genes))
names(common_meta_genes) <- "Genes"

write_tsv(common_meta_genes, file.path("/Data/results/", "trip_neg_common_genes.tsv"))
...
phenoGroups = rep("result", length(meta_dir_paths))
phenoCases = rep(list("tri_neg"), length(meta_dir_paths))
phenoControls = rep(list("non_tri_neg"), length(meta_dir_paths))
...
write_tsv(new_full_results, file.path("/Data/results/", "trip_neg_full_meta_results.tsv"))

source("/prepare_data/functions/draw_Heatmap.R")
png("/Data/results/trip_neg_status_heatmap.png", width = 24, height = 12, units = "in", res = 300)
