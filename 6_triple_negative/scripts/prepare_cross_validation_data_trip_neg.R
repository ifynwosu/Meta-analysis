...
cross_validation_data <- "/Data/cross_validation_data/trip_neg"

if (!dir.exists(cross_validation_data)) {
  dir.create(cross_validation_data, recursive = TRUE)
}

metadata_dir <- "/Data/metadata/triple_negative/"
...
# results_dir <- "/Data/results/HER2_status/"
...
for (i in 1:length(file_paths_meta)) {
    ...
    trip_neg_tally[[i]] <- num_par
    names(trip_neg_tally)[i] <- filename
}

trip_neg_summary <- bind_rows(trip_neg_tally, .id = "Dataset") # convert list to tibble
write_tsv(trip_neg_summary, file.path("/Data/results", "trip_neg_summary.tsv"))
...
# cv_method = kfold
# cross_validation_data <- "/inwosu/Meta_Analysis/Data/cross_validation_meta"
